...
simultaneously with a single shared GRM. A true multi-trait GBLUP is
attempted via `sommer::mmer()` when sommer is installed; if not, or if
the model fails to converge, the function silently falls back to
`rrBLUP::kin.blup()` per trait. Block importance is then aggregated across
traits so rankings are not specific to any single trait:
