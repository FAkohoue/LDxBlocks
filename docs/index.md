...
12. [Scale strategies and
    ...
    - 12.1. [The LDxBlocks_backend
      interface](#id_121-the-ldxblocks_backend-interface)
...
14. [Function reference](#id_14-function-reference)
    ...
    - 14.6. [Utilities](#id_146-utilities)
15. [Output objects](#id_15-output-objects)
    - 15.1. [`run_Big_LD_all_chr()` — block
      table](#id_151-run_big_ld_all_chr--block-table)
    - 15.2. [`tune_LD_params()` — named
      list](#id_152-tune_ld_params--named-list)
    - 15.3. [`extract_haplotypes()` — named
      list](#id_153-extract_haplotypes--named-list)
    - 15.4. [`compute_haplotype_diversity()` —
      data.frame](#id_154-compute_haplotype_diversity--dataframe)
    - 15.5. [`read_geno()` —
      LDxBlocks_backend](#id_155-read_geno--ldxblocks_backend)
...
segmentation algorithm of Kim et al. (2018) with 15 concrete
improvements:
...
SNPs with $`\mathrm{MAF}_i < \tau_{\mathrm{maf}}`$ (default 0.05) are
removed before any LD computation. Monomorphic SNPs (all observations
identical) are removed unconditionally regardless of the MAF threshold.
...
For each candidate cut position $`i`$, the maximum cross-boundary
squared correlation between a left window $`L`$ and right window $`R`$
of half-width `leng` (default 200 SNPs) is evaluated:
...
If no weak-LD boundaries are found within $`5 \times`$`subSegmSize`
SNPs, the algorithm switches to forced equal-size splits placed at the
...
(`CLQcut`, default 0.5). The adjacency matrix is built by
`build_adj_matrix_cpp()` in a single O(p²) compiled pass.
...
**Expected heterozygosity ($`H_e`$):** Nei’s (1973) gene diversity,
...
where $`p_i`$ is the frequency of haplotype $`i`$ and $`n`$ is the
number of individuals with non-missing haplotypes. $`H_e = 0`$ for a
monomorphic block; $`H_e \to 1`$ for many equally frequent haplotypes.
...
$`\log_2 k`$ for equal-frequency haplotypes.

**Dominant haplotype frequency ($`f_{\max}`$):** frequency of the most
...
| [`prepare_gblup_inputs()`](https://FAkohoue.github.io/LDxBlocks/reference/prepare_gblup_inputs.md) | Align phenotype data frame and haplotype GRM for external GBLUP solvers (rrBLUP, BGLR, ASReml-R). See [Phenotype input format](#phenotype-input-format) for accepted column layouts. |
...
[`integrate_gwas_haplotypes()`](https://FAkohoue.github.io/LDxBlocks/reference/integrate_gwas_haplotypes.md)
\| Combine GWAS, variance, and diversity evidence per block. \|  
