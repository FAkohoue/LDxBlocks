...
**Processing stalls after “Segment N/N done” message.** After all
Leiden/Louvain segments complete, `Big_LD()` runs post-segmentation
steps including LD-informed overlap resolution. In v0.3.1+, this uses
`resolve_overlap_cpp()` — a C++ function with BLAS DGEMM scoring that is
thousands of times faster than the original R `.resolve_overlap()`. If
you are using an older version and see the job hang after the last
segment message, update to v0.3.1+ which routes this step through C++
with a 15,000× reduction in per-SNP LD computation cost.
