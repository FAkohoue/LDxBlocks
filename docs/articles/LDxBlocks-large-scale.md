...
A 10 M-marker, 5,000-individual genotype matrix occupies roughly 200 GB
as a dense R `double` matrix. Loading it into RAM before block detection
is impractical on any workstation. LDxBlocks solves this through the
`LDxBlocks_backend` abstraction:
[`run_Big_LD_all_chr()`](https://FAkohoue.github.io/LDxBlocks/reference/run_Big_LD_all_chr.md)
calls `read_chunk(backend, col_idx)` to obtain exactly the SNP window it
needs, leaving the rest of the genome on disk.
