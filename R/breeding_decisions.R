...
#'     \code{normalize = FALSE}, units are phenotype units x allele dosage.}
...
#'     each individual (effect x dosage sum across alleles). Zero when the
...
#'
#' The output includes one row per (individual x block x allele) combination,
...
#'
#' @return Data frame in long format. One row per individual x block x allele
