# ─────────────────────────────────────────────────────────────────────────────
# compute_ld.R  –  Unified LD computation: standard r² or kinship-adjusted rV²
# ─────────────────────────────────────────────────────────────────────────────
# Design:
#   compute_ld()      -- dispatcher: routes to r² or rV² path
#   compute_r2()      -- standard r² via C++ (fast, no kinship)
#   compute_rV2()     -- kinship-adjusted rV² via C++ (structured populations)
#   get_V_inv_sqrt()  -- whitening factor (unchanged, needed for rV²)
...
#'
#' @param X NumericMatrix (individuals × SNPs).
...
#'
#' @return Symmetric p × p numeric matrix, diagonal 0, values in [0, 1].
#'
...
#' correlation (r²) for a window of SNP columns. Missing genotypes (NA) are
#' mean-imputed per column before computation. This is the default LD metric
#' in \pkg{LDxBlocks} and is 10–50× faster than \code{stats::cor()}.
#'
#' @section When to use r² vs rV²:
#' \describe{
#'   \item{r²}{Use for large unstructured datasets (> 500 k markers), random
...
#'   \item{rV²}{Use for highly structured / related populations (livestock,
...
#' }
#'
#' @param X NumericMatrix (individuals × SNPs). Values 0/1/2. NA allowed.
#' @param digits Integer. Rounding decimal places. \code{-1} (default) = none.
#' @param n_threads Integer. OpenMP threads. Default 1.
#'
#' @return Symmetric p × p NumericMatrix, diagonal 0, values in [0, 1].
#'
...
#'
#' @param X NumericMatrix (n × p). Pre-whitened, mean-centred genotype matrix.
#' @param digits Integer. Rounding. \code{-1} = none.
#' @param n_threads Integer. OpenMP threads.
#'
#' @return Symmetric p × p NumericMatrix, diagonal 0.
#'
...
#'
#' @param V Symmetric positive-definite matrix (n × n). Typically a VanRaden
...
#' }
#'
#' @param geno Numeric matrix (individuals × SNPs), 0/1/2.
#' @param method Character. \code{"r2"} or \code{"rV2"}.
#' @param kin_method Character. Whitening decomposition for rV²:
#'   \code{"chol"} or \code{"eigen"}.
#' @param verbose Logical. Print progress.
#'
#' @return Named list:
#'   \item{adj_geno}{n × p numeric matrix ready for \code{compute_ld()}.}
#'   \item{V_inv_sqrt}{n × n whitening matrix, or \code{NULL} for r².}
#'
#' @export
