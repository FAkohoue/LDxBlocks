...
# These names are used inside ggplot2::aes() calls where non-standard
# evaluation means R cannot resolve them statically.
if (getRversion() >= "2.15.1") {
  utils::globalVariables(c(
    # PCA plot (pc_df columns)
    "PC1", "PC2", "pheno",
    # Scree plot (scree_df columns)
    "PC", "PVE", "cum", "selected"
  ))
}
...
# ==============================================================================
# Internal helper: return a well-formed empty result with status field
# ==============================================================================
.empty_assoc_result <- function(
    status          = "no_results",
    traits          = character(0),
    n_pcs_used      = 0L,
    sig_threshold   = 0.05,
...
# Internal helper: check if a column has more than one unique non-NA value
...
#'   Ignored when \code{optimize_pcs = TRUE}.
#'
#' @param optimize_pcs Logical. When \code{TRUE}, automatically selects
#'   \code{n_pcs} by fitting REML null models for
#'   \code{k = 0, 1, \ldots, optimize_pcs_max} and choosing the value that
#'   minimises the criterion set by \code{optimize_method}. Selection uses
#'   only the first trait (the haplotype GRM is shared across all traits).
#'   Default \code{FALSE}: \code{n_pcs} is used directly.
#'
#' @param optimize_pcs_max Integer. Upper bound on the number of PCs evaluated
#'   when \code{optimize_pcs = TRUE}. Default \code{10L}.
#'
#' @param optimize_method Character. Criterion for PC model selection when
#'   \code{optimize_pcs = TRUE}. One of:
#'   \itemize{
#'     \item \code{"bic_lambda"} (\strong{default, recommended}) - hybrid score:
#'       \eqn{|\lambda_{GC} - 1| + 0.01 \times \tilde{BIC}}, where
#'       \eqn{\tilde{BIC}} is the BIC scaled to \eqn{[0,1]} across all
#'       k values. Minimises genomic inflation while BIC breaks ties toward
#'       fewer PCs.
#'     \item \code{"bic"} - minimise the REML BIC of the null model:
#'       \eqn{-2 \cdot \log L + k_{params} \cdot \log n}.
#'     \item \code{"lambda"} - minimise \eqn{|\lambda_{GC} - 1|} alone, where
#'       \eqn{\lambda_{GC}} is estimated from a fast 500-allele scan on
#'       GRM-corrected residuals.
#'   }
#'   Ignored when \code{optimize_pcs = FALSE}.
#'
...
test_block_haplotypes <- function(
  ...
  n_pcs_used   <- 0L
  pc_scores    <- NULL
  eig          <- NULL          # kept for PCA plot
  pc_model_sel <- NULL          # populated when optimize_pcs = TRUE
  auto_pcs     <- is.null(n_pcs)
  n_pcs_req  <- if (auto_pcs) NA_integer_ else as.integer(n_pcs)

  if (auto_pcs || isTRUE(optimize_pcs) || (!auto_pcs && n_pcs_req > 0L)) {
    .log("Eigendecomposing GRM for PC fixed-effect covariates ...")
    eig <- tryCatch(
      eigen(G, symmetric = TRUE),
      error = function(e) {
        warning("GRM eigendecomposition failed; no PCs will be used.", call. = FALSE)
        NULL
      }
    )
    if (is.null(eig)) {
      ...
      if (isTRUE(optimize_pcs)) {
        # -- Model selection over n_pcs = 0..optimize_pcs_max
        # optimize_method controls the criterion:
        #   "bic"        : minimise BIC of the null REML model
        #   "lambda"     : minimise |lambda_GC - 1| (genomic inflation)
        #   "bic_lambda" : hybrid - minimise abs(lambda-1) + 0.01*scaled_BIC
        #                  (recommended for GWAS: BIC tie-breaks lambda near 1)
        # Uses the first trait (GRM is shared across traits).
        .log("  Optimising n_pcs via '", optimize_method,
             "' (0..", optimize_pcs_max, " PCs) ...")
        tr_sel  <- names(blues_list)[1]
        ph_sel  <- blues_list[[tr_sel]]
        com_sel <- intersect(names(ph_sel), rownames(G))
        com_sel <- com_sel[!is.na(ph_sel[com_sel]) & is.finite(ph_sel[com_sel])]
        y_sel   <- ph_sel[com_sel]
        G_sel   <- G[com_sel, com_sel, drop = FALSE]
        ev      <- eig$vectors
        rownames(ev) <- rownames(G)
        n_sel   <- length(y_sel)

        # Per-k fit statistics
        k_seq <- 0L:as.integer(optimize_pcs_max)
        bic_k <- lambda_k <- numeric(length(k_seq))

        for (ki in seq_along(k_seq)) {
          k   <- k_seq[ki]
          X_k <- if (k == 0L) {
            matrix(1, nrow = n_sel, ncol = 1L)
          } else {
            cbind(1, ev[com_sel, seq_len(k), drop = FALSE])
          }
          fit <- tryCatch(
            rrBLUP::mixed.solve(y = y_sel, X = X_k, K = G_sel, method = "REML"),
            error = function(e) NULL
          )
          if (is.null(fit) || !is.finite(fit$LL)) {
            bic_k[ki] <- NA_real_; lambda_k[ki] <- NA_real_; next
          }
          n_param    <- k + 3L   # intercept + k PCs + sigma_g + sigma_e
          bic_k[ki]  <- -2 * fit$LL + n_param * log(n_sel)
          # Compute lambda_GC from residuals under this null model
          y_fixed_k  <- as.numeric(X_k %*% fit$beta)
          u_k        <- fit$u[com_sel]; u_k[is.na(u_k)] <- 0
          resid_k    <- y_sel - y_fixed_k - u_k
          # Approximate lambda via chi-sq median of null residuals
          # (exact computation requires per-allele p-values; use fast proxy:
          #  variance ratio of corrected vs raw phenotype as lambda proxy)
          # Run the vectorised scan on the first 500 alleles for speed
          X_scan_sub <- hap_mat[com_sel, seq_len(min(500L, ncol(hap_mat))),
                                drop = FALSE]
          X_scan_sub[is.na(X_scan_sub)] <- 0
          XtX_s  <- colSums(X_scan_sub^2) - colSums(X_scan_sub)^2 / n_sel
          X_s    <- X_scan_sub[, XtX_s > 1e-10, drop = FALSE]
          XtX_s2 <- XtX_s[XtX_s > 1e-10]
          if (!ncol(X_s)) { lambda_k[ki] <- NA_real_; next }
          xb  <- colMeans(X_s); yb <- mean(resid_k)
          Xty <- as.numeric(crossprod(X_s, resid_k)) - n_sel * xb * yb
          bt  <- Xty / XtX_s2
          ss  <- sum((resid_k - yb)^2)
          rss <- pmax(ss - bt^2 * XtX_s2, 0)
          se  <- sqrt(rss / (n_sel - 2L) / XtX_s2)
          ts  <- bt / se
          ps  <- 2 * stats::pt(-abs(ts), df = n_sel - 2L)
          chisq_obs <- stats::qchisq(ps, df = 1L, lower.tail = FALSE)
          lambda_k[ki] <- stats::median(chisq_obs, na.rm = TRUE) /
            stats::qchisq(0.5, df = 1L, lower.tail = FALSE)
        }

        # Build model selection table
        bic_sc  <- (bic_k  - min(bic_k,  na.rm = TRUE)) /
          (max(bic_k,  na.rm = TRUE) - min(bic_k,  na.rm = TRUE) + 1e-10)
        score_k <- switch(optimize_method,
                          bic        = bic_k,
                          lambda     = abs(lambda_k - 1),
                          bic_lambda = abs(lambda_k - 1) + 0.01 * bic_sc
        )

        pc_model_sel <- data.frame(
          n_pcs     = k_seq,
          BIC       = round(bic_k,    2),
          lambda_gc = round(lambda_k, 4),
          score     = round(score_k,  6),
          selected  = FALSE,
          stringsAsFactors = FALSE
        )

        best_ki    <- which.min(score_k)
        best_k     <- k_seq[best_ki]
        n_pcs_used <- as.integer(best_k)
        pc_model_sel$selected[best_ki] <- TRUE

        .log(sprintf("  Optimal n_pcs = %d  (criterion: %s | lambda_GC = %.3f)",
                     n_pcs_used, optimize_method,
                     pc_model_sel$lambda_gc[best_ki]))
        .log("  Model selection table:")
        for (ri in seq_len(nrow(pc_model_sel))) {
          .log(sprintf("    k=%2d  BIC=%8.1f  lambda=%.3f  score=%.4f%s",
                       pc_model_sel$n_pcs[ri],   pc_model_sel$BIC[ri],
                       pc_model_sel$lambda_gc[ri], pc_model_sel$score[ri],
                       if (pc_model_sel$selected[ri]) "  <-- selected" else ""))
        }
  ...
  for (tr in traits) {
    ...
    ok_y <- !is.na(y) & is.finite(y)

    if (sum(ok_y) < 10L) {
      warning("Fewer than 10 non-missing phenotypes for trait '", tr, "' - skipping.",
              call. = FALSE)
      next
    }
    ...
    if (!is.null(null_fit)) {
      ...
      u_sub <- null_fit$u[common]; u_sub[is.na(u_sub)] <- 0
      y_resid <- y - y_fixed - u_sub
    } else {
      lm_null <- tryCatch(stats::lm.fit(x = X_null, y = y), error = function(e) NULL)
      if (is.null(lm_null)) {
        warning("Fixed-effect fallback model also failed for trait '", tr, "' - skipping.",
                call. = FALSE)
        next
      }
      y_resid <- stats::residuals(lm_null)
    }
    ...
    for (bn in unique_blocks) {
      ...
      blk_meta <- if (!is.null(bi) && is.data.frame(bi)) {
      ...
      blk_var <- apply(X_blk, 2L, .is_polymorphic_col)
      X_blk   <- X_blk[, blk_var, drop = FALSE]
      ...
      if (RSS_null < 1e-15 || df_lrt < 1L || df_res < 1L) {
        block_rows[[length(block_rows) + 1L]] <- data.frame(
          block_id         = bn,
          CHR              = gm2("CHR", NA_character_),
          start_bp         = gm2("start_bp", NA_integer_),
...
.plot_assoc_results <- function(allele_df,
  ...
  # -- PCA plot (once, not per trait) ------------------------------------------
  if (!is.null(eig) && !is.null(eig$vectors) && ncol(eig$vectors) >= 2L) {
    tryCatch({
      eig_vals <- pmax(eig$values, 0)
      var_prop <- eig_vals / sum(eig_vals) * 100

      pc_df <- data.frame(
        id  = rownames(allele_df)[1],   # placeholder; overwritten below
        PC1 = eig$vectors[, 1L],
        PC2 = eig$vectors[, 2L],
        row.names = NULL,
        stringsAsFactors = FALSE
      )
      pc_df$id <- rownames(eig$vectors)

      # Colour by mean phenotype of first trait if available
      colour_by <- "individual"
      if (!is.null(blues_list) && length(blues_list) >= 1L) {
        ph     <- blues_list[[1L]]
        common <- intersect(pc_df$id, names(ph))
        if (length(common) >= 2L) {
          pc_df$pheno <- NA_real_
          pc_df$pheno[match(common, pc_df$id)] <- as.numeric(ph[common])
          colour_by <- names(blues_list)[1L]
        }
      }

      # Mark individuals used as PCs in the model
      pc_df$model_pc <- if (n_pcs_used > 0L) "Used in model" else "Not used"

      p_pca <- if ("pheno" %in% names(pc_df)) {
        ggplot2::ggplot(
          pc_df, ggplot2::aes(x = PC1, y = PC2, colour = pheno)) +
        ...
        ggplot2::ggplot(pc_df, ggplot2::aes(x = PC1, y = PC2)) +
          ggplot2::geom_point(colour = "#6495ED", size = 1.8, alpha = 0.8)
      }

      p_pca <- p_pca +
        ggplot2::labs(
          title    = "PCA - haplotype GRM",
          subtitle = sprintf("n_pcs used in model: %d | PC1 %.1f%% | PC2 %.1f%%",
                             as.integer(n_pcs_used), var_prop[1L], var_prop[2L]),
          x = sprintf("PC1 (%.1f%%)", var_prop[1L]),
          y = sprintf("PC2 (%.1f%%)", var_prop[2L])
        ) +
      ...
      pca_file <- file.path(out_dir, "pca_grm.pdf")
      ggplot2::ggsave(pca_file, p_pca, width = 6, height = 5.5,
                      device = "pdf", useDingbats = FALSE)
      if (verbose) message("[plot] PCA saved: ", pca_file)
    }, error = function(e) {
      if (verbose) message("[plot] PCA skipped: ", conditionMessage(e))
    })
  }
  ...
  if (!is.null(eig) && !is.null(eig$values)) {
    tryCatch({
      eig_vals  <- pmax(eig$values, 0)
      var_prop  <- eig_vals / sum(eig_vals) * 100
      n_show    <- min(30L, length(var_prop))
      scree_df  <- data.frame(
        PC  = seq_len(n_show),
        PVE = var_prop[seq_len(n_show)],
        cum = cumsum(var_prop[seq_len(n_show)]),
        stringsAsFactors = FALSE
      )
      scree_df$selected <- scree_df$PC == (n_pcs_used)
      # Add BIC/lambda selection path if available
      sel_label <- if (!is.null(pc_model_sel) && any(pc_model_sel$selected)) {
        sel_row <- pc_model_sel[pc_model_sel$selected, ]
        sprintf("Selected: k=%d | lambda=%.3f | criterion: score=%.4f",
                sel_row$n_pcs[1], sel_row$lambda_gc[1], sel_row$score[1])
      } else {
        sprintf("n_pcs used in model: %d", as.integer(n_pcs_used))
      }
      ...
      p_scree <- ggplot2::ggplot(scree_df,
        ...
        ggplot2::scale_fill_manual(
          values = c("FALSE" = "#6495ED", "TRUE" = "#800000"),
          guide  = "none") +
        ggplot2::geom_line(
          ggplot2::aes(y = cum), colour = "grey40",
          linetype = "dashed", linewidth = 0.5) +
        ggplot2::geom_vline(
          xintercept = n_pcs_used + 0.5,
          colour = "#800000", linetype = "solid", linewidth = 0.5) +
        ggplot2::scale_x_continuous(breaks = seq(1, n_show, by = 2)) +
        ggplot2::labs(
          title    = "Scree plot - haplotype GRM",
          subtitle = sel_label,
      ...
      scree_file <- file.path(out_dir, "grm_scree.pdf")
      ggplot2::ggsave(scree_file, p_scree, width = 7, height = 4.5,
                      device = "pdf", useDingbats = FALSE)
      if (verbose) message("[plot] Scree plot saved: ", scree_file)
    }, error = function(e) {
      if (verbose) message("[plot] Scree plot skipped: ", conditionMessage(e))
    })
  }
  ...
  for (tr in traits) {
    ...
    p_qq <- ggplot2::ggplot(qq_df, ggplot2::aes(x = expected, y = observed)) +
      ...
      ggplot2::labs(
        ...
        subtitle = sprintf("lambda (GC) = %.3f | %d tests", lambda, n_p),
    ...
    manh_file <- file.path(out_dir, paste0("manhattan_", safe_tr, ".pdf"))
    qq_file   <- file.path(out_dir, paste0("qq_",        safe_tr, ".pdf"))

    ggplot2::ggsave(manh_file, p_manh, width = 14, height = 5.5,
                    device = "pdf", useDingbats = FALSE)
    ggplot2::ggsave(qq_file,   p_qq,   width = 5,  height = 5,
                    device = "pdf", useDingbats = FALSE)
...
print.LDxBlocks_haplotype_assoc <- function(x, ...) {
  ...
  if (!is.null(x$pc_model_selection) && nrow(x$pc_model_selection) > 0) {
    cat("\n  PC model selection (optimize_pcs = TRUE):\n")
    sel <- x$pc_model_selection
    for (i in seq_len(nrow(sel))) {
      cat(sprintf("    k=%2d  BIC=%8.1f  lambda_GC=%.3f  score=%.4f%s\n",
                  sel$n_pcs[i], sel$BIC[i], sel$lambda_gc[i], sel$score[i],
                  if (sel$selected[i]) "  <-- selected" else ""))
    }
  }
  invisible(x)
}
...
#'   flag in \code{$omnibus_tests}. One of:
#'   \itemize{
#'     \item \code{"p_omnibus_adj"} (default) - plain Bonferroni:
#'       \eqn{\min(p \times n_{blocks},\,1)}, computed per trait.
#'       Retained for backward compatibility.
#'     \item \code{"p_omnibus_fdr"} - Benjamini-Hochberg FDR per trait.
#'     \item \code{"p_omnibus_simplem"} - simpleM Bonferroni-style:
#'       \eqn{\min(p \times M_{\mathrm{eff}},\,1)}, where
#'       \eqn{M_{\mathrm{eff}}} is estimated from the eigenspectrum of the
#'       block-summary PC1 matrix.
#'     \item \code{"p_omnibus_simplem_sidak"} - simpleM Sidak-style:
#'       \eqn{1-(1-p)^{M_{\mathrm{eff}}}}. Recommended: consistent with
#'       \code{\link{test_block_haplotypes}}.
#'   }
#'   All four p-value columns are always present in \code{$omnibus_tests}
#'   regardless of this choice.
#'
#' @param meff_percent_cut Numeric in (0, 1). Proportion of variance explained
#'   by the retained PCs in the simpleM eigendecomposition used to estimate
#'   \eqn{M_{\mathrm{eff}}} for \code{sig_metric = "p_omnibus_simplem"} or
#'   \code{"p_omnibus_simplem_sidak"}. Default \code{0.995} (99.5\%),
#'   following the original simpleM recommendation (Gao et al. 2008).
#'
#' @param meff_max_cols Integer. Maximum columns per eigendecomposition chunk
#'   when estimating \eqn{M_{\mathrm{eff}}}. Default \code{1000L}.
#'
...
estimate_diplotype_effects <- function(
    ...
    blue_cols        = NULL,
    sig_threshold    = 0.05,
    sig_metric       = c("p_omnibus_adj", "p_omnibus_fdr",
                         "p_omnibus_simplem", "p_omnibus_simplem_sidak"),
    meff_percent_cut = 0.995,
    meff_max_cols    = 1000L,
    verbose          = TRUE
) {
  ...
  sig_metric_dip <- match.arg(sig_metric)

  omnibus_df <- if (length(omnibus_rows)) {
    df <- do.call(rbind, omnibus_rows)

    # -- Bonferroni (backward-compat, per trait) ------------------------------
    df$p_omnibus_adj <- NA_real_
    for (tr in unique(df$trait)) {
      idx_tr <- which(df$trait == tr)
      df$p_omnibus_adj[idx_tr] <- pmin(df$p_omnibus[idx_tr] * length(idx_tr), 1)
    }

    # -- BH-FDR (per trait) ---------------------------------------------------
    df$p_omnibus_fdr <- NA_real_
    for (tr in unique(df$trait)) {
      idx_tr <- which(df$trait == tr)
      df$p_omnibus_fdr[idx_tr] <- stats::p.adjust(df$p_omnibus[idx_tr], method = "BH")
    }

    # -- simpleM Meff (per trait, global scope across all tested blocks) ------
    # Use the block-level omnibus p-values as the input vector.
    # Meff is estimated from a block-summary correlation matrix derived from
    # the haplotype feature matrix (one PC1 per block), consistent with
    # test_block_haplotypes() block-level Meff.
    # For estimate_diplotype_effects() we use a simpler approximation:
    # the number of tested blocks per trait as Meff (worst-case Bonferroni)
    # unless the hap_mat is available to compute actual LD-based Meff.
    df$p_omnibus_simplem       <- NA_real_
    df$p_omnibus_simplem_sidak <- NA_real_

    for (tr in unique(df$trait)) {
      idx_tr <- which(df$trait == tr)
      n_tr   <- length(idx_tr)
      # Meff from eigenspectrum of the haplotype dosage correlation matrix,
      # using the same block-summary approach as test_block_haplotypes.
      meff_tr <- tryCatch({
        # hap_mat was built above and is in scope
        col_blocks_hm <- sub("_hap\\d+$", "", colnames(hap_mat))
        X_bsum <- .make_block_summary_matrix(hap_mat, col_blocks_hm)
        .compute_simplem_meff_matrix(
          X_bsum, percent_cut = meff_percent_cut, max_cols = meff_max_cols
        )$meff
      }, error = function(e) n_tr)
      if (!is.finite(meff_tr) || meff_tr < 1L) meff_tr <- n_tr

      df$p_omnibus_simplem[idx_tr] <-
        .adjust_p_simplem_bonf(df$p_omnibus[idx_tr], rep(meff_tr, n_tr))
      df$p_omnibus_simplem_sidak[idx_tr] <-
        .adjust_p_simplem_sidak(df$p_omnibus[idx_tr], rep(meff_tr, n_tr))
    }

    # -- Significance flag using chosen sig_metric ----------------------------
    df$significant <- switch(sig_metric_dip,
                             p_omnibus_adj          = df$p_omnibus_adj          < sig_threshold,
                             p_omnibus_fdr          = df$p_omnibus_fdr          < sig_threshold,
                             p_omnibus_simplem      = df$p_omnibus_simplem      < sig_threshold,
                             p_omnibus_simplem_sidak= df$p_omnibus_simplem_sidak< sig_threshold
    )
