...
# Each individual is a diploid combination of two founders, guaranteeing
# sufficient diplotype diversity (AA/AB/BB classes) for all package tests.
make_block <- function(n_ind, n_snp, p = NULL, n_founders = 4L, flip = 0.03) {
  # Step 1: K founder haplotypes (binary vectors of length n_snp)
  founders <- matrix(0L, nrow = n_founders, ncol = n_snp)
  for (k in seq_len(n_founders)) {
    p_k <- runif(1, 0.15, 0.85)
    founders[k, ] <- rbinom(n_snp, 1L, p_k)
  }

  # Step 2: Assign founder pairs to individuals with skewed frequencies
  ...
  # Step 4: Small random flips for realism
...
stale_gds <-## data-raw/generate_example_data.R
  ## ─────────────────────────────────────────────────────────────────────────────
  ## Generates all example datasets shipped with LDxBlocks.
  ## Run once with: source("data-raw/generate_example_data.R")
  ## Outputs land in data/ (as .rda) and inst/extdata/ (as flat files).
...
list.files("inst/extdata", pattern = "\\.gds$", full.names = TRUE)
if (length(stale_gds)) {
  file.remove(stale_gds)
  message("Removed ", length(stale_gds), " stale GDS cache file(s) from inst/extdata/")
}
